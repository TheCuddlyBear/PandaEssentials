...
public class armorstand implements CommandExecutor {
      ...
      if (argument.equals("true") || argument.equals("false")) {
        if (argument.equals("true")) {
          ...
          player.sendMessage(ChatColor.GREEN +
                             "Made ArmorStand's arms visible");
        } else {
          e.setArms(false);
          player.sendMessage(ChatColor.GREEN +
                             "Made ArmorStand's arms invisible");
        }
      } else {
  ...
  public void argumentSetBasePlate(String argument, Player player,
                                   ArmorStand e) {
    if (!argument.isEmpty()) {
      if (argument.equals("true") || argument.equals("false")) {
        if (argument.equals("true")) {
          e.setBasePlate(true);
          player.sendMessage(ChatColor.GREEN +
                             "Made ArmorStand's base plate visible");
        } else {
          e.setBasePlate(false);
          player.sendMessage(ChatColor.GREEN +
                             "Made ArmorStand's base plate invisible");
        }
      } else {
        player.sendMessage(ChatColor.RED + "Invalid Arguments!");
        player.sendMessage(ChatColor.RED +
                           "/armorstand setbaseplate <true/false>");
      }
    } else {
      player.sendMessage(ChatColor.RED + "Invalid Arguments!");
      player.sendMessage(ChatColor.RED +
